...
function readLocal(relPath) {
  const candidates = [
    ...
    path.join(__dirname, 'frontend', relPath.replace(/^frontend\//, '')),
    path.join(__dirname, 'cipher-corridor-monitor', relPath),
    path.join(process.cwd(), relPath),
    path.join(process.cwd(), 'cipher-corridor-monitor', relPath),
    path.join(process.cwd(), 'frontend', relPath.replace(/^frontend\//, '')),
  ];
  ...
  throw new Error(`File not found: ${relPath} (checked: ${candidates.join(', ')})`);
}

const main = readLocal('frontend/src/main.js');
...
const sig = readLocal('frontend/src/signals.js');
...
const dd = readLocal('frontend/src/drawer-detail.js');
...
const ds = readLocal('frontend/src/drawer-scenario.js');
...
const br = readLocal('frontend/src/briefing.js');
...
const md = readLocal('frontend/src/masterdata.js');
...
const au = readLocal('frontend/src/audit.js');
...
const up = readLocal('frontend/src/uploader.js');
...
const css = readLocal('frontend/src/style.css');
...
const html = readLocal('frontend/index.html');
